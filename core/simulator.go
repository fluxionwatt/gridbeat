...
func ServerSimulator(errorLogger *logrus.Logger) {
	...
	for _, db := range Gconfig.Simulator.Devices {
		...
		if err != nil {
			errorLogger.Error(fmt.Sprintf("simulator %s failed to create server at url %s: %v", db.Name, db.URL, err))
			continue
		}
		...
		err = server.Start()
		if err != nil {
			errorLogger.Error(fmt.Sprintf("simulator %s failed to start server at url %s: %v", db.Name, db.URL, err))
			continue
		}

		errorLogger.Info(fmt.Sprintf("simulator %s start at url %s", db.Name, db.URL))
