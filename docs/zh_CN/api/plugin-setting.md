...
```json
{
    ...
    "params": {
        ...
        "timeout": 3000,
         // required, 0 the gridbeat driver is used as the client, 1 the gridbeat driver is used as the server
        "connection_mode": 0
    }
}
```
...
```json
{
    ...
    "params": {
        // required, 0 the gridbeat driver is used as the client, 1 the gridbeat driver is used as the server
        "connection_mode": 0,
        // required, client: host means the ip of the remote device. server: it means the ip used by gridbeat locally
        "host": "127.0.0.1",
        // required, client: port means the tcp port of the remote device. server: it means the tcp port used by gridbeat locally
        "port": 502,
        // required, timeout for sending requests to the device
        "timeout": 3000,
...
```json
{
    ...
    "params": {
        ...
        // required, timeout for sending requests to the device
        "timeout": 3000,
        // tcp required, client: host means the ip of the remote device. server: it means the ip used by gridbeat locally
        "host": "127.0.0.1",
        // tcp required, client: port means the tcp port of the remote device. server: it means the tcp port used by gridbeat locally
        "port": 502,
        // tcp required, 0 the gridbeat driver is used as the client, 1 the gridbeat driver is used as the server
        "connection_mode": 0,
        // required, send reading instruction interval(ms)
        "interval": 20,
        // required, the maximum number of retries after a failed attempt to send a read command
