...
GridBeat 从设备采集到的数据可以通过 Sparkplug B 协议从边缘端传输到 Sparkplug B 应用中，用户也可以从应用程序向 GridBeat 发送数据修改指令。
...
Sparkplug B 是运行在 MQTT 之上的应用型协议，所以在 GridBeat 中的设置与 MQTT 插件相似。
...
您可通过 GridBeat Sparkplug B 插件将数据上报到 EMQX，并通过 EMQX 的编解码功能得到正确完整的数据结果，具体结果，见 [集成 EMQX](sparkplug.md)。

您可通过 GridBeat SparkPlugB 插件连接 Ignition 平台，具体步骤，见 [Ignition](./ignition.md)。

您也可通过 GridBeat SparkPlugB 插件连接 Cogent DataHub，具体步骤，见 [Cogent](./cogent.md)。
