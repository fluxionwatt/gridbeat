...
本节将演示如何通过 GridBeat 南向采集设备实际点位数据，通过北向 Sparkplug B 插件将数据上报到 EMQX， 再通过EMQX编解码功能解码后得到正确完整的数据结果，流程如图：
...
我们可以通过安装 [MQTTX 桌面客户端](https://mqttx.app/zh)查看由 GridBeat 转发到 EMQX 的数据。但如果直接订阅 GridBeat 北向 SparkPlugB 上报到 EMQX 的数据，则会出现字符串乱码的情况，如图：
...
这里的关键点在于 `schema_decode('gridbeat', payload, 'Payload')`：
...
如上图，可以看到解码前的原数据是乱码的，解码后得到完整正确的数据结果；至此，通过GridBeat南向采集设备点位值，北向 SparkPlugB 上报到 EMQX，通过编解码功能解码得到完整的数据结果已完成。

## 拓展阅读：Sparkplug 协议

GridBeat 上报数据到 EMQX 的 Topic 是根据 Sparkplug B 协议规范定义的，格式如下：
...
至于更多 GridBeat 北向 Sparkplug B 插件相关标准的定义，可以参考 [SparkPlug B 协议规范](https://www.eclipse.org/tahu/spec/Sparkplug%20Topic%20Namespace%20and%20State%20ManagementV2.2-with%20appendix%20B%20format%20-%20Eclipse.pdf)。
