...
GridBeat 支持将 OPC UA Server 作为北向应用，以便将南向设备的数据通过 OPC UA 服务暴露给上层系统或第三方客户端。通过 OPC UA Server，外部系统可以订阅数据变化、读取实时点位，以及下发控制命令。
...
当 GridBeat 首次启动 OPC UA Server 会生成自签名证书，外部客户端可能需要手动信任该证书（例如在 UA 客户端中将证书导入受信任列表）。
主动上传的客户端证书默认受信任，陌生客户端连接，证书会加入非信任列表，需要在界面进行手动信任操作。

### 命名与映射规则

GridBeat 会将南向设备中的点位（tag）映射为 OPC UA 的节点（Node）。映射规则如下：

- 每个南向节点（例如 modbus1）对应一个 OPC UA 对象节点（Object）。
- 组（group）作为子对象组织在南向节点下。
- 点位（tag）映射为变量节点（Variable），变量的 DataType 根据 GridBeat 中定义的数据类型映射到 OPC UA 类型（如 Double、Int32、Boolean、String 等）。

所有南向节点都位于 GridBeatEX 节点之下。NodeId 遵循 `ns=1;s=[南向设备名称].[组名称].[点位名称]` 规范，例如 `ns=1;s=modbus-tcp-1.group-1.temperature`，ns=1 代表 GridBeatEX 的命名空间。
