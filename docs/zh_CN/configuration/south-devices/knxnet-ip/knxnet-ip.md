...
GridBeat KNXnet/IP 插件支持与 KNXnet/IP 设备建立连接。

::: tip

由于 KNXnet/IP 协议的工作原理，如果使用虚拟化技术如虚拟机或 Docker 部署 GridBeat，KNX 插件可能
无法正常工作。如果是在 Linux 主机中使用 docker 镜像部署 GridBeat，那么需要使用 docker 选项`--net=host`。
在其他情况下，推荐您使用二进制安装包部署 GridBeat。
...
注意:如果使用多拨地址 *224.0.23.12* 进行配置，通常要求设备与 GridBeat 部署在同一网段中。
