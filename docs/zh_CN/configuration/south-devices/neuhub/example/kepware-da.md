...
本文将使用 GridBeat HUB 插件和 NEURON HUB Windows 程序连接 KEPServerEX 6 OPC DA Server，实现读写 OPC DA Server 的数据。
...
2. 填写 `node name`，此参数为 OPC DA 节点唯一标志，不可重复，后续会在 GridBeat HUB 驱动中使用；
...
OPC DA 节点在正常连接状态下，可以按 GridBeat EX 点位模板文件的格式导出 OPC DA 全部点位。具体操作为将鼠标移动到需要导出点位的 OPC DA 节点，鼠标右键菜单，点击 `export gridbeatex tags`。
 ![export gridbeatex tags](./assets/export_ex_tags.png)


## 使用 GridBeat HUB 驱动连接 OPC DA 节点

* 在 **南向设备** 中 点击 `添加设备`，选择 `GridBeat HUB` 插件来创建一个连接 NEURON HUB Windows 程序的节点。填写配置信息，
