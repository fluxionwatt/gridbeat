...
GridBeat HollySys Modbus TCP 插件使用 Modbus TCP 协议，用于采集和利时 PLC 标签的数据，支持 LK/LE 系列 PLC 型号。
...
| **IP 地址** | TCP 连接时，设备的 IP 地址（GridBeat 作为客户端）。|
| **端口号** | TCP 连接时，设备的端口号（GridBeat 作为客户端）。|
...
GridBeat HollySys Modbus TCP 插件将 PLC 数据单元名称指定为 **ADDRESS**，用户不需要关心地址映射的细节。
