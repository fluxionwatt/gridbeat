...
本节介绍如何通过 GridBeat Mitsubishi 3E 插件连接 FX5U。
...
1. 在 GridBeat 南向设备管理中添加一个 Mitsubishi 3E 设备。
...
## 使用 UDP 模式连接 FX5U
1. 设置 PLC 中的 UDP 协议，**协议**选择 UDP，**端口号**填写 PLC 监听端口，**IP 地址**填写 GridBeat 运行主机的 IP 地址。
![fx5u7](./assets/fx5u7.jpg)

2. 配置 GridBeat UDP 传输模式，**传输模式**选择 UDP，UDP源端口使用默认端口号 52001。
