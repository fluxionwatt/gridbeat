...
## 如何连接作为 Server 的 GridBeat？

本节主要介绍 GridBeat 作为 Server，Modbus Poll 作为 Client 时， GridBeat 与 Modbus Poll 的相关配置。

Modbus Poll 作为 Client，主动向 GridBeat 发起连接请求，用户需要保证 Modbus Poll -> GridBeat 的网络连通性。这种连接方式通常可用于以下场景中，在某些设备使用 4G 上网时，因为 GridBeat 无法主动连接到设备，所以，GridBeat 只能选择 Server 模式，由设备主动连接到 GridBeat。

### 配置 GridBeat 南向驱动 Server

在南向驱动中，使用 Modbus RTU 插件新建一个名为 modbus-rtu-server 的节点，并进行驱动配置，如下图所示。

![gridbeat-rtu-server-config](./assets/gridbeat-rtu-server-config.png)
...
* 进入菜单 **Connection -> Connect**，根据实际情况选择连接方式（本示例为 Modbus RTU/ASCII Over TCP/IP），设置连接参数（GridBeat 运行设备的 IP 和设置的 Port），然后点击 **OK** 完成配置， 如下图所示。
