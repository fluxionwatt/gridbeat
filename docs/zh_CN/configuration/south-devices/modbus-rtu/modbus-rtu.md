...
Modbus RTU 协议采用二进制编码，可以在 RS-232、RS-485 或其他串行通信介质上传输数据，GridBeat 的 Modbus RTU 插件增加了基于以太网 TCP 的实现，通过 DTU 设备，可实现远端设备的数据采集与控制。
...
通过有人云 DTU 透传模块将连接到 GridBeat 为例，将串口数据转换为网络数据，具体步骤，见 [DTU 连接示例](./example/dtu/dtu-connection.md)。
