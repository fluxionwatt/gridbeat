...
| 点位处理      | modbus_point.c modbus_point.h              | GridBeat 中 tag 类型转换为插件所需要的更具体的类型 |
| 驱动层开发    | modbus_tcp.c modbus_req.c modbus_req.h     | 插件主题框架的实现                            |
| 插件设置文件   | modbus-tcp.json                            | 插件设置文件的定义                            |

## 常量说明

头文件 <define.h> 中定义了 GridBeat 中主要的常量。
...
* NEU_PLUGIN_KIND_STATIC，GridBeat 内嵌插件，不需要动态库文件；
* NEU_PLUGIN_KIND_SYSTEM，系统插件，需要以动态库文件方式存储，可由 GridBeat 加载；
* NEU_PLUGIN_KIND_CUSTOM，自定义插件，需要以动态库文件方式存储，可由 GridBeat 加载；
...
这部分主要用于将 GridBeat 中设置的 tag 信息转换为所需要的更具体的信息。

* GridBeat 中点位配置的地址可以进一步解析成设备地址 Slave ID，寄存器类型和寄存器起始地址，并能校验该地址是否符合插件规范。
* 不同的寄存器支持的数据类型不同，这里可以根据解析出来的寄存器类型及 GridBeat 中配置的属性进行判断配置的地址是否支持该属性。
* Modbus 支持批量读取数据，GridBeat 可以把用户配置的点位进行聚合分类，根据分类的结果，可实现批量读取数据的功能。

## 第四步，驱动层开发

modbus_req.c 和 modbus_req.h 文件定义 modbus_tcp.c 文件中使用的具体函数实现及结构体。modbus_tcp.c 文件主要是插件的接口函数，由 gridbeat 中的 plugin.h 定义，plugin_intf_funs 说明如下。

### open

调用 driver_open 函数，基于 plugin 创建 node 时 gridbeat 第一个调用的函数，创建插件自己定义的结构体 struct neu_plugin。该结构体在 modbus_req.h 中定义，需要注意的是结构体中的第一个成员一定是 neu_plugin_common_t common，其他成员可根据驱动的具体实现增加。
...
调用 driver_close 函数，删除 node 时，gridbeat 调用的最后一个函数，用于释放由 open 创建的 neu_plugin_t。
...
调用 driver_init 函数，在创建 node 时，gridbeat 调用完 open 后，紧接着调用的函数。此函数主要做插件内需要初始化的一些资源，modbus 插件中主要初始化 modbus 协议栈解析。其中的回调函数都在 modbus_req 文件中实现。
...
调用 driver_uninit 函数，删除 node 时，gridbeat 首先调用的函数，此函数主要释放一些在 init 中申请以及初始化的资源。
...
调用 driver_start 函数，用户在 gridbeat node 页面，点击开始时，gridbeat 会调用此函数，start 不做任何处理，只返回 0，通知插件开始运行，以及开始连接设备等，连接状态的处理放在 tcp 连接的异步回调函数中处理。

![start](./assets/modbus-start.png)

### stop

调用 driver_stop 函数，用户在 gridbeat node 页面，点击停止时，gridbeat 会调用此函数，stop 通知插件停止运行，关闭与设备之间的连接，并且 driver.group_timer 将不会再触发。

![stop](./assets/modbus-stop.png)

### setting

调用 driver_config 函数，用户在 gridbeat node 设置页面进行设置时使用，node 设置的参数将通过 json 方式呈现，gridbeat 将通过此函数通知插件进行设置。driver_config 函数首先会解析并保存配置信息，然后建立 tcp 连接。modbus_conn_connected 和 modbus_conn_disconnected 两个回调函数会分别在 tcp 建立和关闭连接时被调用，相应的这两个函数会更新插件的连接状态。
...
调用 driver_validate_tag 函数，在向 node 中添加 tag 或更新 tag 时，gridbeat 会把 tag 相关参数使用此函数通知到插件，插件根据各自实现检查此 tag 参数是否符合插件要求，该函数返回 0，代表成功。
...
调用 driver_write 函数，当使用 write api 时，gridbeat 将调用此函数，通知插件，向点位 tag 写入特定的值。
