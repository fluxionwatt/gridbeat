...
GridBeat 代码以高可读性为原则，有以下规范要求：
  ...
  * GridBeat 使用 clang-format 进行代码格式化，clang-format 文件在 GridBeat 根目录。
...
* PR 流程如下：
    ...
    * 先 Fork 本项目，创建自己的 github.com/your/gridbeat 仓库；<br>
    * 克隆自己的 gridbeat 仓库到本地：
    ```bash
    ...
    * 上一步处理完毕后，就可以把自己创建的分支推送到自己的仓库：
    ...
    ```
    * 最后，把自己仓库的新推送的分支往 emqx/gridbeat 对应的分支发 PR 即可；
...
1. 使用日志，打印相关调试信息
2. 因 GridBeat 是多线程异步并发的程序，不推荐使用 GDB 进行断点调试等。
