...
The GridBeat Inovance Modbus TCP plugin is tailored for Inovance PLC tags.
...
::: tip
Since communication is done via TCP/IP, the device is searched through the IP address and port number, making it unnecessary to specify the station number in the message. GridBeat defaults the station number to 1.
:::
...
Using Inovance' PLC programming software AutoShop, you can quickly connect to the PLC and GridBeat for debugging. For specific operations, see [Connect to Easy521](./example/autoshop/autoshop-modbus).
