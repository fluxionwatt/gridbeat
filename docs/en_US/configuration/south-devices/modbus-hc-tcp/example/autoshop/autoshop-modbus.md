...
The Inovance Modbus TCP plugin requires connection to the Inovance PLC via the local area network, so please ensure that the PLC and GridBeat are in the same local area network.
...
## Configure GridBeat
* In GridBeat, click **Configuration** -> **South Devices**. Click on **Add Device** in the **South Devices** and select the Inovance Modbus TCP plugin to create a node to connect to the Easy521 PLC.
