...
* Check if the address is correct (GridBeat Modbus address starts from 1).
* Some devices may not support reading only one point at a time, so all points need to be configured (GridBeat will automatically optimize batch reading).
