...
In addition to supporting data acquisition and processing via TCP client mode, the GridBeat Modbus TCP plugin also supports TCP server mode, which allows devices to connect to GridBeat actively. This feature is mainly used for 4G DTU because the IP address of 4G network is a private IP. In this case, the DTU device can only connect to GridBeat actively.
...
| -------------------- | ------------------------------------------------------- |
| **Connection Mode** | Only for **TCP** mode, When selecting TCP, you can choose GridBeat as the TCP client or server. |
...
| **IP Address** | The IP address of the device when using TCP connection with GridBeat as the client, or the IP address of GridBeat when using TCP connection with GridBeat as the server. The default value is 0.0.0.0. <!--to be confirmed--> |
| **Port** | The port number of the device when using TCP connection with GridBeat as the client, or the port number of GridBeat when using TCP connection with GridBeat as the server.|
| **Connection Timeout** |  The time the system waits for a device to respond to a command. |
| **Check Header** | Choose whether to verify the message header. After selecting True, when encountering packet header errors, the gridbeat and device will reconnect. |
...
After the plugin is added and configured, the next step is to establish communication between your device and GridBeat by adding groups and tags to the Southbound driver.
