...
The GridBeat ADS plugin enables users to connect to Beckhoff TwinCAT PLC over TCP/IP.
...
| Source AdsAmsNetId | AMS Net ID of the machine running gridbeat.                    |
| Source AdsPortNr   | AMS port number of the machine running gridbeat (default 851). |
| Target AdsAmsNetId | AMS Net ID of the target PLC.                                |
| Target AdsPortNr   | AMS port number of the target PLC (default 851).             |

Note that an ADS route corresponding to the parameter setting should be created in the target TwinCAT software, so that GridBeat could correctly communicate with the TwinCAT PLC.
...
YOu can use GridBeat to collect data from Beckhoff software PLCs using the GridBeat ADS plugin, for details, see [Data Acquisition with Beckhoff ADS Plugin](./plc-ads/ads.md).
