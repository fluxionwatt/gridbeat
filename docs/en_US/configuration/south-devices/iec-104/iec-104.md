...
GridBeat supports the acquisition of remote signaling, remote measurement, and remote pulse (electricity call) data in the form of station interrogation, and supports the control of devices through remote control.
...
The point attribute is configured as `Subscribe`, which means that the point will not only regularly report the value of the data point sent by the 104 Server according to the configured `Station Interrogation Interval`, but also when the data point changes, GridBeat will also receive the change data points, and report the data points northbound through the northbound application configuration.

::: tip 
Due to the real-time requirements of the 104 protocol, for points configured as `Subscribe`, GridBeat will immediately report the changed data points to the northbound application after receiving the changes in the data points, and will not be subject to the `interval` of the collection group limit.
:::
...
### Subscribe
IEC60870-5-104 supports the subscription of data changes. In GridBeat, you need to configure the Subscribe attribute for the tag. When the device sends the changed data to GridBeat, GridBeat will send the change message to the corresponding northbound application in time.
...
You can simulate an IEC60870-5-104 device and test its connection to GridBeat using the IEC 60870-5-104 Server Simulator. For specific steps, refer to [IEC 60870-5-104 Server Simulator](./example/server-simulator/simulator.md).
