...
* Check whether the S7 node configuration parameters configured in GridBeat match the PLC, such as CPU rack number (slot), CPU slot number (rack), etc.
 ...
 * Check whether the server where GridBeat is installed can communicate with PLC. If it can ping, you can try to use the telnet tool to test the PLC port.