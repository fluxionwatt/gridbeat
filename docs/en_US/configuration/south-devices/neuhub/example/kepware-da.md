...
This article demonstrates how to use the GridBeat HUB plugin and the NEURON HUB Windows program to connect to the KEPServerEX 6 OPC DA Server, enabling read and write operations on OPC DA Server data.  
...
2. Fill in the `node name`. This parameter serves as the unique identifier for the OPC DA node and cannot be duplicated. It will be used later in the GridBeat HUB driver.  
...
When the OPC DA node is in a normal connected state, all OPC DA tags can be exported in the format of the GridBeat EX tag template file. To do this, hover the mouse over the OPC DA node whose tags need to be exported, right-click, and select `export gridbeatex tags`.  

 ![export gridbeatex tags](./assets/export_ex_tags.png)  

## Connecting to the OPC DA Node Using the GridBeat HUB Driver  

* In the **Southbound Devices** section, click `Add Device`, select the `GridBeat HUB` plugin to create a node connecting to the NEURON HUB Windows program. Fill in the configuration information:  
