...
This section introduces how to use GridBeat KNXnet/IP plugin to communicate with KNXnet/IP. 

::: tip

Due to the way how KNXnet/IP protocol works, the KNX plugin may not be able to work correctly
if GridBeat is installed using some virtualization technology such as virtual machines or docker.
In a Linux host with docker, using the docker option `--net=host` is required. In other cases,
we recommend that you install GridBeat using binary packages.
...
After clicking **Create**, you will be redirected to the **Device Configuration** page, where we will set up the parameters required for GridBeat to establish a connection with the northbound application. You can also click the device configuration icon on the southbound device card to enter the **Device Configuration** interface.
...
Note that setting with the multicast address *224.0.23.12* normally requires that the KNXnet/IP device and GridBeat are in the same subnetwork.
...
- When reading the KNX plugin, GridBeat sends a `GroupValueRead` tunneling request using the specified group address, and updates the tag value upon receiving a `GroupValueResp` matching the specified individual address.
- When writing the KNX plugin, GridBeat sends a `GroupValueWrite` tunneling request using the specified group address.
