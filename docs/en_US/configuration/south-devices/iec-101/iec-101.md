...
After adding and configuring the plugin, you need to establish communication between the device and GridBeat by adding groups and points to the southbound driver.
...
IEC 60870-5-101 supports the point to report the value when the point changes. On the GridBeat side, you need to configure the point with the Subscribe attribute. When the device sends the changed data point to GridBeat, GridBeat will immediately send this change message to the corresponding northbound application.
