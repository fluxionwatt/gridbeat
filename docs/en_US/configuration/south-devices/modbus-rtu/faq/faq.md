...
* Verify if the address is correct (Modbus address for GridBeat starts from 1).
* Some devices may not support reading a single point at a time, so all points need to be configured (GridBeat automatically optimizes batch reading).
...
* The DTU has enabled registration or heartbeat packets, causing GridBeat to consider data flow data as incorrect.
