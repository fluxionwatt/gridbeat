...
The Modbus RTU protocol uses binary encoding and can transmit data over RS-232, RS-485, or other serial communication media. GridBeat Modbus RTU plugin adds an implementation based on Ethernet TCP and enables remote device data acquisition and control through a DTU device.
...
| **Connection Mode** | Only for the **Ethernet** mode, you can choose GridBeat as the TCP client or server. |
| **IP Address** | Only for the **Ethernet** mode,  the IP address of the device when using TCP connection with GridBeat as the client, or the IP address of GridBeat when using TCP connection with GridBeat as the server. The default value is 0.0.0.0. |
| **Port** | Only for the **Ethernet** mode, the port number of the device when using TCP connection with GridBeat as the client, or the port number of GridBeat when using TCP connection with GridBeat as the server. |
