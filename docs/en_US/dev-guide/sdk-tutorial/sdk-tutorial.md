...
This article mainly introduces how to develop a new driver plugin based on the SDK package and apply it to GridBeat.

## Step 1 Download and install the SDK

Download link: [https://github.com/emqx/gridbeat/releases](https://github.com/emqx/gridbeat/releases)

According to different development systems, download the corresponding sdk tar.gz package, e.g. gridbeat-sdk-2.1.3-linux-amd64.tar.gz to the corresponding development system and decompress it to obtain gridbeat-sdk-x.x.x, where x.x.x represents the version number, execute the following command.

```bash
# take version 2.3.0 as an example
$ cd gridbeat-sdk-2.3.0
# install sdk
$ sudo ./sdk-install.sh
```
...
| Path                      | Description                            |
| --------------------------| ------------------------------------------------------------------------------------------------------------------- |
| /usr/local/include/gridbeat | Stores GridBeat header files and apply to include_directories in the CMakeLists.txt compilation file                   |
| /usr/local/lib/gridbeat     | Stores GridBeat dependent library files, which are applied to link_directories in the CMakeLists.txt compilation file |
| /usr/local/bin/gridbeat     | Holds files needed to run GridBeat             |
...
# add the path to the gridbeat header
include_directories(/usr/local/include /usr/local/include/gridbeat)
# add the path to the gridbeat library
link_directories(/usr/local/lib /usr/local/lib/gridbeat)
...
target_link_libraries(${MODBUS_TCP_PLUGIN} gridbeat-base)
```
...
| -------------------- | --------------------------------- |
| .open                | The first function called by gridbeat when creating a node based on the plugin, create a struct neu_plugin defined by the plugin itself |
| .close               | The last function called by gridbeat when a node is removed to release the neu_plugin_t created by open |
| .init                | When creating a node, after gridbeat calls open, the function called immediately after that. This function is mainly used for some resources that need to be initialized in the plugin |
| .uninit              | The function that gridbeat calls first when deleting a node. This function mainly releases some resources applied and initialized in init |
| .start               | On the node page, set the job status to start, gridbeat will call this function to notify the plugin to start running, and start connecting to the device |
| .stop                | On the node page, set the working status to stop, gridbeat will call this function to notify the plugin to stop running, close the connection with the device, and driver.group_timer will no longer trigger |
| .setting             | On the node page, when the plugin is set, the parameters will be set in json format, and gridbeat will notify the plugin to set through this function |
| .request             | This function has not been used in the development of southbound driver |
| .driver.validate_tag | When a node adds or updates a tag, gridbeat will use this function to notify the plug-in of the relevant parameters of the tag. The plug-in checks whether the tag meets the requirements of the plug-in according to its own implementation. The function returns 0, which means success |
...
| .driver.write_tag    | When using the write API, gridbeat calls this function to notify the plugin to write a specific value to the point tag |
...
## Step 3 The plugin is applied to GridBeat

### Copy the driver .so file

After compiling, go to modbus/build/plugins and copy the generated driver .so file (for example, libplugin-modbus-tcp.so ) to the /usr/local/bin/gridbeat/plugins directory.

### Copy the driver configuration .json file

Copy the modbus/modbus-tcp.json file to the /usr/local/bin/gridbeat/plugins/schema directory.

### Modify plugins.json file

Open the plugins.json file in the /usr/local/bin/gridbeat/persistence directory and add the name of the newly added driver .so file to it.

### Start GridBeat verification driver

Go back to the /usr/local/bin/gridbeat directory and execute the following command to run GridBeat.

```bash
$ sudo ./gridbeat --log
```

Open GridBeat on the web page to view the added plugins and their usage.
